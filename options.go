...
type Options struct {
	...
	Experimental struct {
		...
		// SharedFS is a second file system that can be shared between multiple
		// Pebble instances. It must only contain sstables. Each sstable might only
		// be written to by one Pebble instance, but other Pebble instances can
		// possibly read the same files if they have the path to get to them. The
		// pebble instance that wrote a file should not delete it if other Pebble
		// instances are known to be reading this file. This FS is expected to have
		// slower read/write performance than the default FS above.
		...
		// this instance from that of others in SharedFS. Defaults to a random
...
func (o *Options) EnsureDefaults() *Options {
	...
	if o.Experimental.InstanceID == 0 {
		o.Experimental.InstanceID = rand.Uint64()
	}
	...
	return parseOptions(s, func(section, key, value string) error {
		...
		case section == "Options":
			...
			case "instance_id":
				...
				instanceID, err = strconv.ParseUint(value, 10, 64)
				o.Experimental.InstanceID = instanceID
...
func (o *Options) checkOptions(s string) (strictWALTail bool, instanceID uint64, err error) {
