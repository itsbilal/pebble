...
func (d *DB) runCompaction(
	...
	if d.opts.Experimental.SharedFS != nil && c.outputLevel.level >= 5 {
		...
		for _, nf := range ve.NewFiles {
			fs := d.opts.FS
			sharedFileHandle := vfs.SharedFileHandle{
				FileNum:           uint64(nf.Meta.FileNum),
				CreatorInstanceID: d.opts.Experimental.InstanceID,
			}
			sharedFS := d.opts.Experimental.SharedFS
			origPath := base.MakeFilepath(d.opts.FS, d.dirname, fileTypeTable, nf.Meta.FileNum)
			destPath := sharedFS.ResolvePath(sharedFileHandle)
...
func (d *DB) paceAndDeleteObsoleteFiles(jobID int, files []obsoleteFile) {
	...
	for _, of := range files {
		...
		if of.info.onSharedFS {
			sharedFileHandle := vfs.SharedFileHandle{
				FileNum:           uint64(of.info.fileNum),
				CreatorInstanceID: of.info.creatorID,
			}
			_ = d.opts.Experimental.SharedFS.MarkObsolete(sharedFileHandle)
		} else {
