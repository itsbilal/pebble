...
func newTableCacheContainer(
	tc *TableCache, cacheID uint64, dirname string, fs vfs.FS, sharedDir string, sharedFS vfs.FS, psCache *persistentCache, opts *Options, size int,
) *tableCacheContainer {
	...
	t.dbOpts.fs = fs
	t.dbOpts.sharedDir = sharedDir
	t.dbOpts.sharedFS = sharedFS
	t.dbOpts.uniqueID = opts.UniqueID
	t.dbOpts.psCache = psCache
