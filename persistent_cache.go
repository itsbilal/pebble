...
func newPersistentCache(localFS vfs.FS, localDir string, sharedFS vfs.FS, sharedDir string, uniqueID uint16) *persistentCache {
	psc := &persistentCache{
...
func (l *persistentCache) copyAsync(localPath string, sharedPath string, val *persistentCacheValue) {
	defer l.wg.Done()
	defer val.Unref()
